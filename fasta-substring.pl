...
while (<$in>){
    ...
    if (m/>/){                      # if the line has a > in it then its a description
        my $des = $_;
        my ($name) = $des =~ m/(\w+)/;  # extract the gene name from description w=alphanumeric (doesn't work if theres weird punctuation)
        $names{$name}="na";
        my @keys = keys(%names);
    }
}
...
while (my $entry = $fasta->nextEntry) {
	...
	my ($fname) = $def =~ m/(\w+)/;   # extract name from definition line (fname=fasta name)
