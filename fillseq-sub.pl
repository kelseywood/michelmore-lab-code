...
sub fill{
    my ($des, $motif) = @_;
    my ($name, $pos) = $des =~ /(\w+)\/(\d+)/; #extract the gene name from description -(\w+ = one or more letters, numbers or _; \/ = forward slash; \d+ digits)
...
sub count_dashes{
    ...
    if ($motif =~ /^\w/){$count = 0} #if motif starts with a character instead of a dash then dash count = 0
        ...
        my ($dashes) = $motif =~ /(^\-+)\w+/;
        $count = ($dashes =~ tr/\-/\-/);    #use transliterate to count dashes (replace - with -)
    }
    return $count;
}
