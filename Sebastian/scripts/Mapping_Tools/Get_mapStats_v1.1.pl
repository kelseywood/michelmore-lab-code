...
# Read in the loci information
open(LOC, $locFile) or die "Can't open $locFile";
...
# Analysis loci for double recombinants
...
# Analyzing data per individual

open(INDIVIDUALCOUNTS, ">$outputPrefix.countsPerIndividual.loc");
...
# Analyzing data per marker

open(MARKERCOUNTS, ">$outputPrefix.countsPerMarker.loc");
...
# Printing to main stats file

open(STATS, ">$outputPrefix.stats.txt");
