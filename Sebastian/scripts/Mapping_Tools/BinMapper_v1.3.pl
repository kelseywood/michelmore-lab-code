...
while ($binline = <BINFILE>) {
	...
	$contigCode = substr(@markerInfo[1], 0, 4);

	$classes{$contigCode} = @markerInfo[0];
...
while ($mapline = <MAPFILE>) {
	...
	$binCode = substr(@posInfo[1], 0, 4);

	$curClass = $classes{$binCode};

	@markers = @bins{$curClass};

	foreach (@{$bins{$curClass}}) {

		print RESULTS $posInfo[0], "\tLG_",$posInfo[0],"-", $curClass, "\t", $_, "\t", $posInfo[2], "\n";
