...
#    if ( ($ref_startPos-52 < 1) ||  ($ref_endPos+52 > $seq->length) ) {
#        return;
#    }
    my $left_flank = $seq->subseq(($ref_startPos-52), ($ref_startPos));
    my $right_flank = $seq->subseq(($ref_startPos+1), ($ref_endPos+52));
