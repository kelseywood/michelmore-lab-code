...
open(SNP, ">$ARGV[5]_minAlleleFreq_$alleleFreq.mindepth_$minDepth.SNP.txt");
open(HETS, ">$ARGV[5]_minAlleleFreq_$alleleFreq.mindepth_$minDepth.Heterozygotes.txt");
open(INDELS, ">$ARGV[5]_minAlleleFreq_$alleleFreq.mindepth_$minDepth.Indels.txt");
open(INDELSPOS, ">$ARGV[5]_minAlleleFreq_$alleleFreq.mindepth_$minDepth.IndelsOnlyPosition.txt");
...
while ($snpLine = <GENOTABLE>) {
	...
	chop $snpLine;
        ...
        $allele_ref = $snpData[2];
        $allele_consensus = $snpData[3];
	...
	$depth = $depthGeno1 + $depthGeno2 + $depthGeno3;
        ...
        foreach $elem (@all_allele) {
		  ...
		  if (($elem eq $refAllele) && ($all_depth[$curGeno] >= $minDepth)){
	              $consensusTrueCover = 1;
                  }

                  if (($elem ne $refAllele) && ($all_depth[$curGeno] >= $minDepth)){
                    if ($elem eq "A" || $elem eq "C" || $elem eq "G" || $elem eq "T"){
	              $polimorphicTrueCover = 1;
                    }
                  }
...
#         print "\n";
        ...
        # skip the enitre line if consensus is equal eaqual to reference
        if ($allele_ref eq $allele_consensus){
            print INDELS "$snpLine\n";
            print INDELSPOS "$snpData[0]\t$snpData[1]\n";
            next;
            }
	...
	$hetCount = 0;
		...
		if($consensusTrueCover == 1 && $polimorphicTrueCover  == 1) {
