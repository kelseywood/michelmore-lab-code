...
print $fh_ill join(",", ('SNP_Name', 'Contig', 'Pos', 'Sequence', 'Maor_Allele', 'Maor_Depth', 'EJ_Allele', 'EJ_Depth', 'CM334_Allele', 'CM334_Depth') )."\n";
...
while (<$fh>) {
    #handle output here
    my ($SNPid, $contig, $pos, $ref_base, $snp_base, $depth, $Maor_base, $Maor_depth, $EJ_base, $EJ_depth, $CM334_base, $CM334_depth, undef ) = split(/\t/);
    print STDERR "$contig\t$pos\t$ref_base\t$snp_base\n";
    ...
    print_illumina_line($SNPid, $contig, $pos, $snp_seq, $Maor_base, $Maor_depth, $EJ_base, $EJ_depth, $CM334_base, $CM334_depth);
}
close($fh);
close($fh_ill);
...
sub create_snp_sequence {
    ...
    if ( ($ref_pos-50 < 1) ||  ($ref_pos+50 > $seq->length) ) {
        return;
    }
    my $left_flank = $seq->subseq(($ref_pos-50), ($ref_pos-1));
    my $right_flank = $seq->subseq(($ref_pos+1), ($ref_pos+50));
...
sub print_illumina_line {
    my ($SNPid, $contig, $pos, $sequence, $Maor_base, $Maor_depth, $EJ_base, $EJ_depth, $CM334_base, $CM334_depth) = @_;
    #SNP_name sequence genome_build_ver chr coordinate source dbsnp_ver ploidy, species, Customer strand
    print $fh_ill join(",", ($SNPid, $contig, $pos, $sequence, $Maor_base, $Maor_depth, $EJ_base, $EJ_depth, $CM334_base, $CM334_depth) )."\n";
    return;
}