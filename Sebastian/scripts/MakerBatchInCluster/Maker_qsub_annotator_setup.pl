...
paste $tmp/$fasta.name.txt $tmp/$fasta.hostname.txt $tmp/$fasta.date.txt >> ../maker_threads.log
maker -nodatastore -cpus $threads $again $force -genome ../@{$fastas{$fasta}}[0] ../maker_opts.ctl ../maker_exe.ctl ../maker_bopts.ctl 2>> ../maker_run_logs/@{$fastas{$fasta}}[1].log
rm -r $fasta.maker.output/mpi_blastdb/
find_gff.pl $fasta.maker.output ../GFF_files YES
