...
} elsif ( (!defined($inputFile) && !defined($qsubsFile)) || !(defined($gffprefix)) || ( ($inputFile =~ /.*\\.((f|F)(a|A))|((f|F)(a|A)(s|S)(t|T)(a|A))$/) && !defined($numseq) ) || !(defined($workingDir)) ) {

	print "Missing require arguments\n";
	print "Usage:\n";
	print "maker_automatic_annotator_v1.pl -input <FASTA/TXT> -gffprefix <prefix> -numseq <INT> -gffprefix <prefix for outputgff> -workdir <Directory where is goning to run> other_options\n";
...
$maker_variables{'TMP'} = "$workingDir/maker_runs/tmp";
...
system("mkdir maker_runs/tmp") unless -d 'maker_runs/tmp';
...
if(!defined($qsubsFile)) {
	...
	print RUNLOG "Maker_qsub_annotator_setu_v2.pl $inputFile $numseq $amountseq $threads $again $force $gffprefix $workingDir\n\n";

	if ( system("Maker_qsub_annotator_setup_v2.pl $inputFile $numseq $amountseq $threads $again $force $gffprefix $workingDir") != 0 ) { die "FATAL ERRROR: Maker_qsub_annotator_setup_v2.pl failed\n"}
...
workdir		working directory where the system is going to work
