...
while(my $datastore = readdir(RUNSRESULTS) ) {
	...
	if($datastore =~ /datastore$/) {
				...
				if($nodatastore eq "YES") {
					...
					while( my $result = readdir(SEQUENCERESULT) ) {
						...
						if( $result =~ /.gff$/) {
							...
							system("cp $makerRun/$datastore/$subdatastore/$result $outDir/GFF_files/$result");
						...
						while( my $sequence = readdir(SUBDIRTWO) ) {
							...
							while( my $result = readdir(SEQUENCERESULT) ) {
								...
								if( $result =~ /.gff$/) {
									...
									system("cp $makerRun/$datastore/$subdatastore/$subdirtwo/$sequence/$result $outDir/GFF_files/$result");
		...
		system("cp $makerRun/$datastore $outDir/maker_datastore_logs/$makerRun.master_datastore_index.log");
