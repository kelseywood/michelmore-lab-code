...
# This script was designed to prepare all the files to run maker
...
my $model_org;
my $rmlib;
my $repeat_protein = "/opt/apps/maker/2.30/data/te_proteins.fasta";
my $rm_gff;
...
} elsif ( (!defined($inputFile) && !defined($qsubsFile)) || !(defined($gffprefix)) || ( ($inputFile =~ /.*\\.((f|F)(a|A))|((f|F)(a|A)(s|S)(t|T)(a|A))$/) && !defined($numseq) ) || (!defined($account) ) || (!defined($queue) ) ) {

	print "Missing require arguments\n";
	print "Usage:\n";
	print "maker_automatic_annotator_v1.pl -input <FASTA/TXT> -gffprefix <prefix> -numseq <INT> -gffprefix <prefix for outputgff> other_options\n";
...
system("mkdir $gffprefix") unless -d $gffprefix;
chdir($gffprefix);
...
printLog("Num Sequences	= $numseq\n");
printLog("Max sequence	= $amountseq Mb\n");


printLog("Num threads	= $threads\n");
printLog("Queue		= $queue\n\n");
printLog("Account to charge	= $account\n\n");
...
if( $nohup eq "no") {
	...
	print  "Analysis will be perform in the $gffprefix, all output files and directories will be written there, do you wish to continue?? {Y/N} : ";
	my $answer = <STDIN>;
	if ($answer =~ /^n(?:o)?$/i) {
...
if(!defined($qsubsFile)) {
	...
	print RUNLOG "Maker_qsub_annotator_setu_v2.pl $inputFile $numseq $amountseq '$threads' $again $force $queue $account '$email' $gffprefix $tmp\n\n";

	if ( system("Maker_qsub_annotator_setup_v2.pl $inputFile $numseq $amountseq '$threads' $again $force $queue $account '$email' $gffprefix $tmp") != 0 ) { die "FATAL ERRROR: Maker_qsub_annotator_setup_v2.pl failed\n"}
...
gffprefix	prefix for gff files and name for outputfolder
...
amountseq	amount of sequence in megabase that will be allow per each file
...
account		account to which to charge the jobs
email		email to which send job information
