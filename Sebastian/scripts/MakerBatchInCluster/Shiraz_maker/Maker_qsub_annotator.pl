...
if(!defined($qsubsFile)) {
	...
	print RUNLOG "Maker_qsub_annotator_setup.pl $inputFile $numseq $amountseq $threads $again $force $tmp\n\n";

	if ( system("Maker_qsub_annotator_setup.pl $inputFile $numseq $amountseq $threads $again $force $tmp") != 0 ) { die "FATAL ERRROR: Maker_qsub_annotator_setup.pl failed\n"}
