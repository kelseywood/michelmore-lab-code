...
if (!defined($qsubsFile) || !defined($requiredTime)) {
	print "Required arguments are missing\n";
	print "Usage Launch_qsubs_v2.pl -qsubsFile=<qsub file list> -requiredTime=<HH:MM:SS>\n";
	die "Missing arguments\n";
} #end if missing arguments

system("module swap mvapich2 openmpi");
system("module load maker");
system("module load bioperl");
...
while(my $qsub = <QSUBS>) {
	...
	if (system("sbatch -l h_rt=$requiredTime $qsub >> maker_launchJobs.log")  != 0 ) {
		...
		print MAKERTHREADS $qsub, " submitted  on ", getTime(), " with $requiredTime time\n";
