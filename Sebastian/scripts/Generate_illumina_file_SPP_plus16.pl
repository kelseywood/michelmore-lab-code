...
sub create_snp_sequence {
    ...
    if ( ($ref_startPos-16 < 1) ||  ($ref_endPos+16 > $seq->length) ) {
        return;
    }
    my $left_flank = $seq->subseq(($ref_startPos-16), ($ref_startPos));
    my $right_flank = $seq->subseq(($ref_startPos+1), ($ref_endPos+16));
