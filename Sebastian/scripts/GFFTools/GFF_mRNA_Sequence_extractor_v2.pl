...
#		print ERRORS $mRNA, "\n";
		...
		# Print left flank if required and exon does not start in 1
		if( ($flank > 0) && ( $ExonStarts[0] > 1 ) ) {

			my $leftflank;

			if($ExonStarts[0]-$flank > 1) { $leftflank = lc($seq->subseq($ExonStarts[0]-$flank, $ExonStarts[0]-1)) }
			else { $leftflank = lc($seq->subseq(1, $ExonStarts[0]-1)) }
...
#			print STDERR $mRNA, "\n";
		...
		# Print right flank if required
		if( ($flank > 0) && ($prevEnd+1 < ($seq->length) ) ) {

			my $rightflank;

			if( $prevEnd+$flank < ($seq->length) ) { $rightflank = lc($seq->subseq($prevEnd+1, $prevEnd+$flank)) }
			else { $rightflank = lc($seq->subseq($prevEnd+1, ($seq->length) )) }

			push(@genomic, "()");
			push(@genomic, $rightflank);

		} #end printing right flank
