...
while (my $line2 = <GFFTOCHECK>) {
	...
	if ( ($GFF_line2[2] eq "exon") || ($GFF_line2[2] eq "CDS") ) {
		...
		$mRNAs{$GFF_line2[0]}{$name}{$GFF_line2[2]}{$GFF_line2[4]}{$GFF_line2[3]} = $GFF_line2[8];
...
open(STILLMISSING, ">$ARGV[2].stillMsissingExons.gff3") or die "Can't open output file\n";
...
foreach my $scaffold (keys %mRNAs) {
	...
	foreach my $mRNA (keys %{$mRNAs{$scaffold}}) {
		...
		if($lastCDSthing[0] ne $firstCDS) {delete($mRNAs{$scaffold}{$mRNA}{'CDS'}{$lastCDSthing[0]})} #end


		if( !defined($mRNAs{$scaffold}{$mRNA}{'exon'}{$firstCDSthing[0]}) ) {

			if( !defined($ExonPool{$scaffold}{$firstCDS}) ) {

				print STILLMISSING "Exon ", $mRNA, " ", $firstCDS, " wasn't found on backup\n";

                        } elsif ( (!keys %{$ExonPool{$scaffold}{$firstCDS}}) ) {

				print STILLMISSING "Exon ", $mRNA, " ", $firstCDS, " wasn't found on backup, empty hash\n";

			} else {

				my @Exonthing = keys %{$ExonPool{$scaffold}{$firstCDS}};

				$mRNAs{$scaffold}{$mRNA}{'CDS'}{$firstCDS}{$firstCDSthing[0]} =~ s/cds/exon/g;
				print MISSING $ExonPool{$scaffold}{$firstCDS}{$Exonthing[0]}, "\t", $mRNAs{$scaffold}{$mRNA}{'CDS'}{$firstCDS}{$firstCDSthing[0]}, "\n";

                               	delete($mRNAs{$scaffold}{$mRNA}{'CDS'}{$firstCDS});
                                delete($mRNAs{$scaffold}{$mRNA}{'CDS'}{$firstCDSthing[0]});
...
#                                	print $mRNA, "\t", $CDS, "\t", $CDSthing[0], "\t", keys %{$mRNAs{$scaffold}{$mRNA}{'CDS'}{$CDS}}, "\n";
					...
					delete($mRNAs{$scaffold}{$mRNA}{'CDS'}{$CDSthing[0]});
