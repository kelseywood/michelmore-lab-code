...
use warnings;
use Getopt::Std;
use vars qw($opt_h $opt_l $opt_x $opt_o $opt_w $opt_D $opt_f $opt_r);
getopts('hl:x:o:w:a:D:f:r');
...
#
# Added score calculation using weights
#
### Difference from version 2.1
#
# Remove annotated flag
# Remove printing of Evidence_status folder
#
...
print LOG "Command line : Exon_evidence_counter_v2.1.pl -l $sourcesFile -x $adj -o $output ";
...
if (defined($opt_w)) {
	...
	open(SCORES,">$output/Scores_perGeneModel.txt");
	close(SCORES);

	open(WEIGHTS, $opt_w) or die "Can't open $opt_w\n";

	my $header = "#";

	while($header =~ m/^#/) { $header = <WEIGHTS>}
	...
	while (my $weightLine = <WEIGHTS>) {

		chomp($weightLine);

		my @weightInfo = split ("\t", $weightLine);

		my $weightCategory = shift(@weightInfo);

		for(my $i = 0; $i < scalar(@types); ++$i) {

			$weights{$weightCategory}{$types[$i]} = $weightInfo[$i];

		} #end for

	} #end reading weights file

	print "Finish reading weights file\n\n";

#	foreach my $category (keys %weights) {
#		print $category;
#		foreach my $weight (keys %{$weights{$category}}) {
#			print "\t", $weights{$category}{$weight};
#		} #end foreach
#		print "\n";
#	} #end foreach

} #end if weights file provided
			...
			for(my $i = 0; $i <= 4; ++$i) {
				...
				if ($exons_gene eq $i || ( ($i eq 4) && ($exons_gene >= $i) ) ) {
					...
					foreach my $category (@sources) {
						...
						if ($datasets{$category} eq "GeneModels") {$adjustment = 0}
...
#			                print "Summarizing evidence\n";
	...
	#Modify function to use the datasets hash
					...
					#Calculate Score and print if weights provided
					if(defined($opt_w) ) {
						open(SCORES,">>$output/Scores_perGeneModel.txt");
						calculate_scores($geneModel, $exons_gene, \@sources, \%weights, \%datasets, \%gene_model_total_stats);
						close(SCORES);
					} #end if scores calculation
...
#	                                        print "$start_start $start_end $end_start $end_end\n";
			...
			if($category eq "GeneModels") {

				print OVERLAP $geneModel,"\t",$category,"\t",$transcript,"\tOppositeStrands\t",($exons_transcript+1), "\t",($exons_gene+1), "\t0\t0\t0\n";

			} #end if it's a gene models
...
sub calculate_scores {
	...
	my ($geneModel, $exons_gene, $sources_reference, $weights_reference, $datasets_reference, $gene_model_total_stats_reference) = @_;

        my @sources = @$sources_reference,

	my %weights = %$weights_reference;
			...
			# if is not a repeat
                	if( $datasets{$category}[1] ne "Repeats" && $datasets{$category}[1] ne "Transposons" ) {
                        	...
                        	foreach my $stat (@statsExons) {
	                            	...
	                            	if( defined($geneModel_stats{$category}{"exons"}{$i}{$stat}) && defined($weights{$category}{$stat}) ) {
			                        $exonScore += $geneModel_stats{$category}{"exons"}{$i}{$stat}*$weights{$category}{$stat};
                        	        } #end else match exons

#					if(!defined($weights{$category}{$stat})) {print STDERR $category, "\t", $stat,"\n"}
                            	...
                            	if( defined($geneModel_stats{$category}{"exons"}{$i}) ) {
					if(!defined($weights{$category}{'matched_exons'})) {
						print "Problem with $category for ".$datasets{$category}[1]."\n";
					} else { $exonScore += $geneModel_stats{$category}{"exons"}{$i}*$weights{$category}{'matched_exons'}; }
                                } #end else match exons
		...
		push(@scores, $exonScore);

		$sumScores += $exonScore;

	} #end for

	my @sortedScores = sort {$a <=> $b} @scores;

	my $averageScore = $sumScores/($exons_gene+1);

	my $midExon = $exons_gene/2;

	my $median;

	if($midExon < 1) { $median = $sortedScores[0] }
	if($midExon == 1) {$median = ($sortedScores[0]+$sortedScores[1])/2}
	elsif(int($midExon) == $midExon ) { $median = $sortedScores[$midExon]}
	else { $median = ($sortedScores[$midExon]+$sortedScores[$midExon+1])/2}

	print SCORES $geneModel, "\t", sprintf("%.2f", $averageScore), "\t", sprintf("%.2f", $median), "\t", join("\t",@scores), "\n";
