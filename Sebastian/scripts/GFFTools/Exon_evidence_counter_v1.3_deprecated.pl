...
use warnings;
use Getopt::Std;
use vars qw($opt_h $opt_l $opt_x $opt_o $opt_a $opt_c $opt_w $opt_D $opt_f);
getopts('hcl:x:o:a:D:f:');
...
############################
#
# Add the weighting calculation system
#
...
if (defined($opt_h)) {
	...
	die "
	usage: .pl -l source-types.txt -x 5 -a string -c -w weights_table.txt -o output -f/-D
	options:
	  ...
	  -D <string>   Directory with the files to analyze (bacth mode) (/home/user/directory/)
          ...
          -c            Calculation of weights table (requires -a)
...
my %total_evidence_counts;

my $weighting = "FALSE";

if( defined($opt_c) ) {

	print "Weights table will be create base on annotated gene models.\n";

	if ( !(defined($opt_a)) ) {

		print "Please provide an annotation flag.\n";

		die;

	} #end if annotation flag is missing

	$weighting = "TRUE";

} # end if trigger weights calculation	
				...
				if($fields[2] eq "mRNA") {
					...
					foreach my $attribute (@attributes) {
						...
						if ($attribute eq $annotationFlag) {
							...
							last if $annotated eq "Annotated";
...
#				print $gene_exon_structure[0][4], "\n";
				...
				if($gene_exon_structure[0][4] eq "Annotated") {
				...
				if($weighting eq "TRUE") {

					calculate_total_counts($gene_exon_structure[0][4],$exons_gene, \@categories, \%gene_model_total_stats)

				} #end if for weighting
...
#        foreach my $key (sort (keys %total_evidence_counts) ) {
#        	foreach my $key2 (sort (keys %{$total_evidence_counts{$key}} ) ) {
#	        	foreach my $key3 (sort (keys %{$total_evidence_counts{$key}{$key2}} ) ) {
#		        	foreach my $key4 (sort (keys %{$total_evidence_counts{$key}{$key2}{$key3}} ) ) {
#			        	print $key, " ", $key2, " ",  $key3, " ",  $key4, " ", $total_evidence_counts{$key}{$key2}{$key3}{$key4}, "\n";
#				} #end foreach
#                        } #end foreach
#                } #end foreach
#        } #end foreach

	open(WEIGHTS, "$output/weights_table.txt");
	...
	my @annotated_gene_model_lengths = (sort {$a<=>$b} keys(%{$total_evidence_counts{'Annotated'}}) );

	my @nonAnnotated_gene_model_lengths = (sort {$a<=>$b} keys(%{$total_evidence_counts{'NonAnnotated'}}) );

	my $maxAnnotated = pop(@annotated_gene_model_lengths);
	my $maxNonAnnotated = pop(@nonAnnotated_gene_model_lengths);

	my $maxNumberExons;

	if($maxAnnotated > $maxNonAnnotated) {

		$maxNumberExons = $maxAnnotated;

	} else {

		$maxNumberExons = $maxNonAnnotated;

	} #end else

	for(my $i = 0; $i <= $maxNumberExons; ++$i) {
			...
			foreach my $category (@categories) {
				...
				if( $category ne "Repeats" && $category ne "Transposons" ) {
					...
					foreach my $stat (@stats) {
						...
						if(defined($total_evidence_counts{'Annotated'}{$i}{$y}{$category}{$stat}) ) {

							$annotatedCount = $total_evidence_counts{'Annotated'}{$i}{$y}{$category}{$stat};
						...
						if(defined($total_evidence_counts{'NonAnnotated'}{$i}{$y}{$category}{$stat}) ) {

							$totalCount = $total_evidence_counts{'NonAnnotated'}{$i}{$y}{$category}{$stat} + $annotatedCount;
	
						} else { 

							$totalCount = $total_evidence_counts{'Annotated'}{$i}{$y}{$category}{$stat};
						...
						if ($totalCount > 0) {
							...
							$weight = $annotatedCount/$totalCount;

						} # end if

						print "Annotated  $i $y $category $stat $weight\n";
						...
						if( defined($total_evidence_counts{'Annotated'}{$i}{$y}{$category}) ) {
						...
						if(defined($total_evidence_counts{'NonAnnotated'}{$i}{$y}{$category}) ) {

							$totalCount = $total_evidence_counts{'NonAnnotated'}{$i}{$y}{$category} + $annotatedCount;
						...
						print "Annotated  $i $y $category $weight\n";

				} #end if for repeats and transposons

			} #end categories foreach

		} #end internal exon loop

	} #end loop of exon lenght


} #end if for calculation of weights	
...
#	my (@gene_exon_structure) = @$gene_exon_structure_reference;
...
#						++$total_evidence_counts{$gene_exon_structure[0][4]}{$exons_gene}{$category}{$stat};
...
#					++$total_evidence_counts{$gene_exon_structure[0][4]}{$exons_gene}{$category};
...
sub calculate_total_counts {

	my ($annotated, $exons_gene, $categories_reference, $gene_model_total_stats_reference) = @_;
	...
	for(my $i = 0; $i <= $exons_gene; ++$i) {
        	...
        	foreach my $category (@categories) {
                	...
                	if( $category ne "Repeats" && $category ne "Transposons" ) {
						...
						++$total_evidence_counts{$annotated}{$exons_gene}{$i}{$category}{$stat};
					...
					++$total_evidence_counts{$annotated}{$exons_gene}{$i}{$category};
