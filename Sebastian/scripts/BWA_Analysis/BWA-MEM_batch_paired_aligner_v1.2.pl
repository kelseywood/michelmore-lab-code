...
use warnings;
use Getopt::Std;
use vars qw($opt_h $opt_f $opt_d $opt_r $opt_B $opt_t $opt_k $opt_M $opt_G $opt_w $opt_v);
getopts('hf:d:r:B:t:k:G:w:v');



### Differences from v0
#
# Flagging of secondary aligments has been made default
# Added option to not generate bam files (speed up alignment process)
#
...
if( (defined($opt_h)) ) {
	...
	print "-B (yes/no) Generate BAM file from SAM file (Default = 'yes')\n";
...
print LOG "Generate BAM files: ", $makeBAM, "\n";
...
while(my $line = <INPUT>) {
	...
	} #end generating BAM files if requested
