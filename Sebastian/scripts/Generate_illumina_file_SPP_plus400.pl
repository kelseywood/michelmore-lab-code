...
sub create_snp_sequence {
    ...
    if ($ref_startPos-400 < 1) {
	$left_flank = $seq->subseq(1, ($ref_startPos));
    } else {

    	print $ref_startPos;
	$left_flank = $seq->subseq(($ref_startPos-400), ($ref_startPos));
    }

    if ($ref_endPos+400 > $seq->length) {
	$right_flank = $seq->subseq(($ref_startPos+1), ($seq->length));
    } else {
    	$right_flank = $seq->subseq(($ref_startPos+1), ($ref_endPos+400));
    }
