       ...
       %posData = ();
        ...
        while ($line = <INPUT>) {
        	...
        	chop($line);
                ...
                $posData{$posID} = [$posInfo[4], $posInfo[5], $posInfo[6], $posInfo[7]];
        ...
        if ($file == 1) {
        	...
        	open(BASEFILE, "Genotype_Table_Indels.txt");
        ...
        while ($baseLine = <BASEFILE>) {
	        ...
	        @baseData = split("\t", $baseLine);
                ...
                if($posData{$posID}[0] ne "") {
	                ...
	                print RESULTS $baseLine, "\t", $posData{$posID}[0], "\t", $posData{$posID}[1], "\t", $posData{$posID}[2], "\t", $posData{$posID}[3], "\n";

                } else {

                        print RESULTS $baseLine, "\tN\t0\tN\t0\n";
        ...
        if(system("mv Temporary_Genotype_Table.txt Genotype_Table_Indels.txt") != 0) {
