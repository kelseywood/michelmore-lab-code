...
while($PIPLine = <PROBEPIP>) {
	...
	if($blocks{$PIPInfo[1]}[0] ne "") {
                ...
                foreach $coord (@{$blocks{$PIPInfo[1]}}) {

                	@cordinateSet = split("-", $coord);

			$startPos = ($PIPInfo[2] + $cordinateSet[1] - 13);

			$endPos = ($PIPInfo[2] + $cordinateSet[1] + 11);

			print GFF3FILE "LG", $cordinateSet[0],"\t.\tAffyProbe\t", $startPos, "\t", $endPos,"\t.\t.\t.\tName=", $PIPInfo[0], ";Contig=", $PIPInfo[1], "\t\t";
			print GFF3FILE "\n";
